...
# gui
    def acessa(self, conta_id:int):  
        while True:
            ...
            if not self.data_recebimento and str(self.data_recebimento_esperado) > date.today().strftime('%Y-%m-%d'):
                print('[1] Receber receita.')
            else:
                if self.data_recebimento:
                    print(f'Essa receita já foi recebida em {self.data_recebimento}.')
                else:
                    print('Essa receita não possui data de recebimento! Edite a receita para que ela possua a data')
            ...
            if decisao == 2:                
                ...
                data_recebimento = Receita.input_data_recebimento()
                
                if valor and data_recebimento_esperado and tipo and data_recebimento:
    ...
    def get_situacao(self):
        if self.data_recebimento == None and str(self.data_recebimento_esperado) > date.today().strftime('%Y-%m-%d'):
            ...
            if self.data_recebimento:
                ...
                if str(self.data_recebimento_esperado) < date.today().strftime('%Y-%m-%d'):
                    return 'Receita cadastrada anterior ao dia atual'
